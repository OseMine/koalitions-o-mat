...
- [x] **`svgBar()` leerer Wrapper** – jetzt real genutzt: `buildResultCardSVG()` baut die Ranglisten-Balken der Ergebnis-Karte über `svgBar()` (Array → Join statt Inline-Konkatenation, Ausgabe identisch). Verifiziert per `node --check` und Harness.
