...
- [x] **Dealbreaker / Rote Linien** – Erweiterung der bestehenden „Wichtige Frage (zählt doppelt)": Thesen als unverhandelbar markieren → Parteien/Koalitionen, die dort gegen den Nutzer stehen, werden stark abgewertet (P2). **Erledigt (Issue #107, 2026-08-10)**: `dealbreakerQuestions` + `frageGewicht()` (Dealbreaker-Gewicht aus `config.thresholds.dealbreakerWeight`, Standard 4; erweitert die Doppelgewichtung, ersetzt sie nicht), ⛔-Button je Frage (`toggleDealbreaker`), Persistenz (`saveTestState`/Share-Hash `&d=`), Konflikt-Abwertung in `berechneUserMatch()` (refactorierte Einzelwert-Logik) & `berechneUserMatchFuerKoalition()`, Konflikt-Badge + Hinweis in den Ergebnissen, i18n-Keys (`dealbreakerHint`/`dealbreakerActiveHint`/`dealbreakerConflict` + `methodologyNote`) inkl. Einfache-Sprache. Verifiziert per `harness/dealbreaker-harness.js` (25/25 Checks); Bericht `reports/implementation-2026-08-10-dealbreaker.md`. → in `archived-todo.md`.
