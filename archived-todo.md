...
### P2 – Koalitions-Reibungs-Index (Friction Score)

- [x] **Koalitions-Reibungs-Index (Friction Score)** – je Koalition zusätzlich zur Übereinstimmung ein Kompromiss-Schwierigkeits-Score. `berechneReibung(parteien)` (script.js) liefert `{ score, konflikte }`: Eine These ist eine Konfliktthese, sobald mindestens ein Parteienpaar direkt gegeneinander steht (j vs. n); der Score ist der Anteil der vergleichbaren (j/n-)Fragen mit mindestens einem solchen Paar (0–100 %). `konflikte` listet die Top-Konfliktthesen sortiert nach der Zahl der streitenden Paare (größte Positionsdifferenz zuerst), je Paar mit konkreten Gegenpositionen (`getAnswerValue`-basiert, neutral „m" trägt nicht bei). Angezeigt: `reibung`-Feld in jeder Koalitions-Karte (`updateKoalitionen()`), Score als `meta-item friction-score`-Badge mit Tooltip (`frictionTitle`), aufklappbare Top-3-Konfliktthesen pro Koalition über `.friction-toggle`-Button (`toggleKoalitionFriction`, `reibungDetailHTML`). Reibungs-Index ist das Alleinstellungsmerkmal: Er macht sichtbar, *warum* eine Koalition schwierig ist.

### P2 – Regierungs-Simulator (Custom Coalition Builder)

- [x] **Regierungs-Simulator** – eigene Koalition manuell per Checkboxen zusammenstellen (`.party-checkboxes` in `#simulatorPartiesCheckboxes`, kein Drag&Drop, passend zur Framework-freien Architektur). `populateSimulatorParties()` befüllt die Auswahl (Parteien der Wahl mit Antworten, inkl. unter-Sperrklausel-Parteien mit Positionen; Auswahl bleibt beim Wahl-/Sprachwechsel erhalten), `renderSimulator()` rendert bei jeder Änderung (change-Listener, `aria-live="polite"`): Sitzmehrheit via `berechneSitze()` (Kombination aus Sitzen → Mehrheit / fehlende Sitze), prozentualer Gesamtwert, interne Übereinstimmung (`berechneUebereinstimmung`), Min. Paar, „Mit Ihnen"-Wert (`berechneUserMatchFuerKoalition`) und Reibungs-Index. Zusätzlich `berechneKoalitionsAbweichung(parteiNames)`: pro Frage zählt, welcher Partner von der Mehrheitsposition der Koalition abweichen müsste (j/n unter den Antwortenden); die Ausgabe zeigt den „Reibungspunkt" (Partner mit den meisten Abweichungen) mit Top-Thesen und den Positionen der anderen Partner. Ausgeschlossene Paare (z. B. AfD+SPD) werden gewarnt, aber nicht blockiert (`.simulator-warning`). Hinweis bei <2 ausgewählten Parteien statt Crash. Koalitions-Simulator steckt im Koalitionen-Tab (`data-simple-off="tab.koalitionen"` ausgeblendet im einfachen Modus).

### P3 – Thesis-Matrix-Heatmap

- [x] **Thesis-Matrix-Heatmap** – Partei × These-Tabelle in Grün/Rot/Grau für schnelle Block-Erkennung. `renderThesisHeatmap()` (Daten & Charts-Tab, `.heatmap-section`) baut aus `fragen.json` (`antworten.wert` je Partei je Frage, vorhanden) und `werteData.umfragewerte` eine kompakte Tabelle: Zeilen = Fragen gruppiert nach Thema (Reihenfolge: `config.topics`, dann Sonstiges), Spalten = Parteien mit Antworten (7 je Wahl, „Andere" ausgeschlossen). Farben sind nie alleiniges Unterscheidungsmerkmal: jede Zelle trägt eine Markierung (✓/✗/·), `title`/`aria-label`-Tooltip (Partei + Frage + Position) sowie das `.heat-legend`-Legenden-Reihe. Zugriff über `.heat-agree`/`.heat-disagree`/`.heat-neutral` (CSS-Variablen, Dark-Mode-tauglich). Nur Rendering neu – die Datenlage war vorhanden. Eingebunden in `initializeDaten()`.

### P3 – Ergebnis-Karte als PNG/SVG exportieren

- [x] **Ergebnis-Karte als PNG/SVG exportieren** – aus den vorhandenen Share-/Ergebnis-Daten eine Social-Media-taugliche Karte (1200×1560): `exportCardData()` liefert Top-Partei (`berechneUserMatchRanking()`), Top-3-Schwerpunktthemen (`topicMatches`) und beste Koalition (gleiche Logik wie `showTestResults()`: Mehrheit, max. Größe, Mindestübereinstimmung, Ausschluss-Checkboxen). `buildResultCardSVG()` rendert reines SVG (kein echarts): Ranglisten-Balken der Top-5-Parteien mit Farben, Beste-Koalition-Chips mit Parteinamen, Schwerpunktthemen-Balken, Header in Partei-Aufmachung, Footer mit App-Name/Datum – Text über `t()` (i18n). `exportResultCard(format)`: SVG direkt als Blob-Download, PNG über `Image`-Objekt → `canvas` (2×-Skalierung für Schärfe) → `toBlob`; Implementierung über `downloadBlob()`. In der Ergebnis-Ansicht als `.tr-export-section` mit zwei Buttons (🖼️ PNG / 📐 SVG); deaktiviert im einfachen Modus (guard `simpleOff('teilen')`), Fehlerpfade mit `.exportCardEmpty`/`.exportCardError`-Meldungen.

### P3 – Share-State live in die URL schreiben

- [x] **Share-State live in die URL schreiben** – statt den Hash nur beim expliziten „Teilen"-Klick zu setzen, wird der Zustand (Wahl, Antworten inkl. Neutral, wichtige/unverhandelbare Fragen, aktive Koalitions-Filter) laufend über `history.replaceState` synchron gehalten (`syncShareUrl()`), ohne die Seiten-Historie zu zersplittern. `buildShareUrl()` baut den Teilen-Link aus dem App-Zustand (kompakte Kodierung `0j1n3j`, `&i=`/`&d=`, `&c=` für die Koalitions-Filter-Sicht; null-safe). `lastSyncedHash` verhindert Overwrite bei unverändertem Zustand; `markHashHandled()` nach externem Hash-Wechsel (geteilter Link/Browser-Zurück) stellt sicher, dass fremde Hashes nicht überschrieben werden. `syncShareUrl()` feuert in `selectAnswer`, `toggleImportant`, `toggleDealbreaker`, `resetTest`, `updateKoalitionen()` und `showTestResults()` – Fortsetzen/Teilen funktioniert damit auch per Bookmark/„URL kopieren", ohne die Historie zu verschmutzen. `parseShareHash()` bleibt rückwärtskompatibel (altes `:`/`,`-Format und `&p=`). Im Einfachen Modus ausgeblendet (guard `simpleOff('teilen')`).

## Implementierung vom 2026-08-11 (Review „Modus & config.json" – 5 Befunde behoben)

Alle fünf Befunde aus `reports/review-2026-08-11-c.md` am selben Tag behoben und **empirisch per Headless-Chromium gegen den realen Build verifiziert** (lokaler Static-Server; `setMode()`/`switchTab()`/`getComputedStyle`; ausgeführtes Skript prüfte P1-1 bis P2-1, alle `true`/erwartet). `node --check script.js` OK.

- [x] **Erweitert-Modus: alle Tab-Panels gleichzeitig sichtbar** – `styles.css:1052` `body:not(.mode-simple) [data-simple-off] { display: revert; }` überschrieb `.tab-content { display:none }` (Specificity), sodass alle vier Panels `display:block` zeigten und der Tab-Wechsel nichts ausblendete. Fix: Regel entfernt und per Kommentar erklärt – `data-simple-off`-Elemente erscheinen ohne `body.mode-simple` von selbst (`.tab-content`/`.tab-content.active` bzw. `.chart-card { display:flex }` liefern die korrekte Darstellung). Verifiziert: nach `setMode('advanced')` nur `#test-content` `block`, alle anderen Panels `none`.
- [x] **Wechsel zurück auf „Einfach" bei aktivem Nicht-Test-Tab → leere tote Hauptfläche** – `applyModeVisibility()` (script.js:70) prüfte `simpleOff('tab.<aktiverTab>')` (Config-Liste, ohne `tab.*`-Keys → `false`). Fix: Fallback prüft jetzt **unabhängig von der Config-Liste** das `data-simple-off`-Attribut des aktiven Tab-Buttons (dieselbe Quelle wie die CSS-Regel): `activeTabBtn.hasAttribute('data-simple-off')` → `switchTab('test')`. Verifiziert: nach `switchTab('daten')`+`setMode('simple')` ist der aktive Tab `test` mit `block`.
- [x] **`config.ui.simple.off` steuert ausgeblendete Ansichten faktisch nicht** – Liste war auf `["kompass","dealbreaker","taktik"]` reduziert (Commit `963144e`) und damit wirkungslos für die HTML-Attribut-Features. Fix: **eine Quelle der Wahrheit** – `applySimpleOffFromConfig()` (script.js:97) leitet die `data-simple-off`-Attribute aus der Config-Liste ab (Attribut für gelistete Keys entfernen, sonst setzen; Grundlage eine **einmalige Momentaufnahme** der Kandidaten aus index.html, damit entfernte Attribute bei zurückkehrendem Key wieder gesetzt werden) und wird in `applyModeVisibility()` nach Config-Load ausgeführt. Gesamt-Liste in `config.json` wiederhergestellt (`tab.parteien`, `tab.koalitionen`, `tab.daten`, `parteiSeite`, `thesenMatrix`, `kompass`, `dealbreaker`, `taktik`, `historie`, `teilen`). Verifiziert: leere Liste → `#tab-parteien` ohne Attribut und Share-Button sichtbar (`inline-block`); Restore der Liste → Attribut wieder gesetzt, Share/`#daten-content` `none`.
- [x] **Dead Guards `simpleOff('tab.*')`/`teilen`/`historie` feuern nie** – durch die wiederhergestellte Gesamt-Liste in `config.json` sprechen `switchTab()` (350), `initializeParteienPage()` (621), `initializeDaten()` (2571), `shareResults()` (194), `saveTestResult()` (2473), `openPartyPage()` (687) wieder auf `true` an (Historie wird im einfachen Modus nicht mehr geschrieben, Teilen gesperrt). Dieselbe Quelle wie die CSS-Ausblendung (Config-Liste). Verifiziert: `simpleOff('tab.parteien'|'teilen'|'historie'|'tab.daten')` → alle `true` in simple mode, `false` in advanced (per Assertion im Headless-Skript).
- [x] **README/Doku beschreiben eine `ui.simple.off`-Steuerung, die so nicht existiert** – durch die Wiederherstellung der Gesamt-Liste und die neue Ableitung der Attribute aus der Config sind README:17/41 und archived-todo.md:46 wieder akkurat: Die Liste steuert jetzt real Tabs, Partei-Detailseiten, Kompass, Thesen-Matrix, Dealbreaker, Taktik, Historie, Teilen.
...
- [x] **Issue „Taktik-Simulator ignoriert `koalitionsausschluss`" (2026-08-09)** – umgesetzt: `calculateTacticalVoting()` (script.js) prüft das Top-2-Paar der Nutzer-Wunschkoalition jetzt mit `istKoalitionAusgeschlossen()` (script.js:1117), konsistent mit `berechneKoalitionen()` (script.js:1157). Ausgeschlossene Paare (z. B. AfD+GRÜNE in LSA, AfD+SPD/GRÜNE/LINKE in allen 4 Wahlen) erzeugen keine Leihstimmen-Warnung und keine konkrete Koalitionsempfehlung (`info.coalition` bleibt `null`, keine „Koalitionsmehrheit"-Anzeige), sondern einen textlichen Tipp (`type: 'excluded'`, gerendert als „Hinweis"). Zulässige Paare behalten ihren unveränderten Loan-/Mehrheits-Pfad. Verifiziert per Node-Harness über alle 4 Wahlen × alle 168 Top-2-Paare (24 ausgeschlossen): 0 Loan-Warnungen auf ausgeschlossenen Paaren, 100 % mit textlichem Tipp, kein `info.coalition` auf ausgeschlossenen Paaren; LSA AfD+GRÜNE liefert nur noch den Tipp. `node --check script.js` OK.
...
- [x] **Issue #55 „Zeilenreferenzen verschoben (kosmetisch)"** – geschlossen (aktuelle Referenzen aktualisiert; historische Reports bleiben Momentaufnahmen). Falls gewünscht als won't-fix dokumentieren.
