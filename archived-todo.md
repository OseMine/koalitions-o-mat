...
Alle fünf Befunde aus `reports/review-2026-08-11.md` am selben Tag behoben und **empirisch per Headless-Chromium gegen den realen Build verifiziert** (lokaler Static-Server; `setMode()`/`switchTab()`/`getComputedStyle`; ausgeführtes Skript prüfte P1-1 bis P2-1, alle `true`/erwartet). `node --check script.js` OK.
